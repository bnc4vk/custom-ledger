...
- Device-based saved-ledger history on the home page with no user profile required
...
- `device_id` (text)
- `ledger_id` (uuid, foreign key to `ledgers.id`)
- `relationship` (`created` or `visited`)
- `created_at` (timestamp)
...
- Saved-ledger history is keyed by a random device id in browser local storage. Ledgers are saved when generated by this browser or visited from this browser. Clearing browser storage or switching devices will hide the local history, but saved ledger URLs still work.
